# -*- coding: utf-8 -*-
"""
Multi-Task Random Forest (MTL-RF) for Alfalfa Quality Estimation
...
This script implements a multi‑task Random Forest regressor that jointly predicts
four alfalfa quality traits (N, CP, ADF, NDF) from multi‑temporal UAV multispectral
features.
...
2. (Optional) Standardize features using sklearn.preprocessing.StandardScaler.

3. Call `train_mtl_rf()` to train the model, or load a pre‑trained model.
...
def train_mtl_rf(X_train, y_train, X_test=None, y_test=None,
                 n_estimators=100, max_depth=6, min_samples_split=4,
                 random_state=42):
    """
    Train a Multi‑Task Random Forest regressor.
...
if __name__ == "__main__":
    print("=== Multi‑Task Random Forest (MTL-RF) ===")
    ...
    # Train model
    model, pred_tr, pred_te, metrics = train_mtl_rf(X_tr, y_tr, X_te, y_te)
    ...
    # Optional: save model
    # import joblib
    # joblib.dump(model, 'mtl_rf_model.pkl')